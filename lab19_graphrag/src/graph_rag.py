"""
GraphRAG query engine: question → entity extraction → BFS on knowledge graph →
subgraph textualization → LLM answer.

Run as:
    python -m src.graph_rag

Requires:
    outputs/triples.json  (produced by src.indexing)
    outputs/graph.graphml (produced by src.graph_networkx)
"""
...
BFS_DEPTH = 2
FUZZY_THRESHOLD = 70          # lower than indexing threshold — question entities are noisier
MAX_FACTS = 60                # cap subgraph facts to avoid context overflow
...
def load_adjacency(triples: list[dict[str, str]]) -> dict[str, list[tuple[str, str]]]:
    ...
    for t in triples:
        ...
        # Also traverse reverse direction so we can follow "CEO_OF" from the org
...
def textualize_subgraph(facts: list[tuple[str, str, str]]) -> str:
    ...
    Example:
        1. OpenAI --[FOUNDED_BY]--> Sam Altman
        2. Sam Altman --[CEO_OF]--> OpenAI
    """
    lines = [
        f"{i + 1}. {s} --[{r}]--> {o}"
...
def extract_question_entities(question: str) -> list[str]:
    ...
    raw = llm.chat(messages, stage="graph_rag_entity_extraction", temperature=0.0, max_tokens=200)
    try:
...
def _load_graph() -> tuple[list[dict[str, str]], dict, list[str]]:
    ...
    if _triples_cache is None:
        ...
        logger.info(
            "Loaded graph: %d triples, %d unique nodes.",
            len(_triples_cache),
            len(_nodes_cache),
        )
...
def query_graph_rag(question: str) -> dict[str, Any]:
    """
    ...
    Steps:
        ...
        3. BFS up to depth=2 from matched nodes.
    ...
    if facts:
        ...
        # Fallback: no graph context found — model will likely say "I don't know"
    ...
    answer = llm.chat(messages, stage="graph_rag_query", temperature=0.0, max_tokens=512)
    ...
    logger.info("GraphRAG answered in %.0f ms (%d tokens).", total_latency_ms, tokens)
    return {
        "answer": answer,
        "tokens": tokens,
...
if __name__ == "__main__":
    ...
    if not TRIPLES_PATH.exists():
        print("ERROR: Run 'python -m src.indexing' first to generate triples.json")
        sys.exit(1)

    demo_q = "Who is the CEO of the company that acquired DeepMind?"
    print(f"\nDemo question: {demo_q}")
    result = query_graph_rag(demo_q)
